<template>
  <div class="flex inline relative gap-6 items-start">
      ...
      <div class="flex relative gap-6 items-start justify-between">
          ...
          <div class="bg-white rounded-2xl card-test-shadows overflow-hidden">
            ...
            <div v-show="openQuestions" class="px-4 pb-4">
              <VueDraggable v-model="questionTypes" :group="{ name: 'questions', pull: 'clone', put: false }"
                ...
                <div v-for="type in questionTypes" :key="type.value" class="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-light
          ...
          <div class="bg-white rounded-2xl card-test-shadows overflow-hidden">
            ...
            <div v-show="openReportBlocks" class="px-4 pb-4 flex flex-col gap-3">
              ...
              <VueDraggable v-model="reportBlockTypes" :group="{ name: 'report-blocks', pull: 'clone', put: false }"
                ...
                <div v-for="type in reportBlockTypes" :key="type.value" class="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-light
               hover:border-green-bright hover:text-green-dark transition G-M text-gray-medium text-sm
               cursor-grab active:cursor-grabbing select-none">
                  <span>{{ type.icon }}</span>
                  <span>{{ type.label }}</span>
              ...
              <div v-if="store.metrics.length > 0">
                ...
                <div class="max-h-48 overflow-y-auto pr-1">
                  <VueDraggable v-model="store.metrics" :group="{ name: 'report-blocks', pull: 'clone', put: false }"
                    ...
                    <div v-for="metric in store.metrics" :key="metric.id" class="flex items-center gap-2 px-3 py-2 rounded-lg border border-green-light bg-bg-light
        ...
        <TestBuilder class="w-full" ref="builderRef" />
      </div>
    </div>
